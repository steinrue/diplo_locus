"""
Python library for HMM functions to compute per-site likelihood based on diffusion_core.py
"""
import numpy
import scipy.stats
...
class SelectionGrid:
    ...
    def __init__ (self, Ne, s1s, s2s, mAlpha, mBeta, selGridSize=11, numStates=1001):
        ...
        yGrid = diffusion_core.getLinearGrid (numStates)
        yBoundaries = diffusion_core.getLinearBounds (yGrid)
...
class SelHmm:
    """Constructor of diploid WF diffusion HMM.
    Construct a diploid Wright-Fisher diffusion HMM, configures its population genetic parameters, and
    ...
    initDist : array_like
        Array(s) of probability densities at each allele frequency gridpoint on each locus.

    Methods
    --------
    __init__(Ne, s1, s2, mAlpha, mBeta, initCond, initFreq=None, initMAlpha=None,
            initMBeta=None, initS1=None, initS2=None, sampleSizesSet=None,
            numStates=1001, deltaT=1, emissionType="integer", transitionType="constant",
    ...
    def getInitDistribution (yGrid, gridWeights, Ne, s1, s2, mAlpha, mBeta, initCond,
        ...
        # easy enough to have it here
            ...
            assert (False), f'Unknown initial condition {initCond}. Please choose between "statBeta", "uniform", and "initFreq"'
    ...
    def __init__ (self, Ne=None, s1=None, s2=None, mAlpha=None, mBeta=None, initCond=None,
        ...
        Initializes object to compute likelihood of observing temporal sampled allele frequency data under the given parameters. Likelihood is computed assuming that underlying population allele frequency evolves according to Wright-Fisher diffusion.
        ...
        s1 : float or array_like
            Selection coefficients of the heterozygote. One value if selection is constant. When simulating piecewise time-varying selection, the number of values should be `selectionChangeTimes + 1`


        s2 : float or array_like
            Selection coefficients of the homozygote. One value if selection is constant. When simulating piecewise time-varying selection, the number of values should be `selectionChangeTimes + 1`
        ...
        initCond : {"uniform", "initFreq", "statBeta"}
            ...
            - "uniform" :
                Distribution of initial frequency in HMM is uniform on [0,1].
            - "initFreq" :
                Specify a fixed frequency as the initial condition for the HMM. The parameter `initFreq` specifies the
                initial frequency(ies). Either one frequency for the analysis of all datasets or one frequency for each
                dataset (has to be compatible with the number of datasets given).
            - "statBeta" :
                Uses the stationary Beta distribution as the initial distribution of the HMM. The mutation rates to
                specify this distribution are `mAlpha` and `mBeta`, ot they can be specified separately using
                `initMAlpha` and `initMBeta`.
        ...
        emissionType : {"integer", "fractional"}, default="integer"
            ...
            - "fractional" :
                Data can have (positive) fractional samples sizes and allele counts. This is useful in situations
                where allele count is estimated and can assume non-integer values.

        transitionType : {"constant", "piecewise"}, default="constant"
            Type of transitions to consider in the HMM:
            - "constant" :
                Selection coefficients stay constant throughout the entire duration considered. `s1` and `s2` must
                be floats (not `array_like`) for this option.
            - "piecewise" :
                This has to be chosen if selection coefficients change over time, in a piece-wise manner. Must also
                specify `selectionChangeTimes` for this option, and `s1` and `s2` must have matching numbers of coefficients.
        ...
        sampleSizesSet : set of array_like
            Required if ``emissionType="integer"``.  Set of list, numpy array, or tuple objects of the
            same length as the number of sampling time points that summarizes all possible sample sizes.
        ...
        assert (mAlpha is not None)
        assert (mBeta is not None)
        assert (initCond is not None)

        # the states
        self.numStates = numStates
        yGrid = diffusion_core.getLinearGrid (numStates)
        boundaries = diffusion_core.getLinearBounds (yGrid)
        gridWeights = diffusion_core.getWeights (boundaries) 
...
def surfaceOneD (times, samplesSizes, samples, selDirection, selCoefficients, **kwargs):
    """
    Computes likelihoods on a one dimension grid of selection coefficients.

    Parameters `times`, `samplesSizes`, and `samples` are the same as for `computeLogLikelihood`.

    Population genetic parameters are the same (named) parameters as for `SelHmm()`. Except selection coefficients, those are handled here.

    Parameters
    ----------
    selDirection : array_like
        Two dimensional vector that gives the direction of the one dimensional surface in (s_1, s_2)-space.

    selCoefficients : array_like
        The coefficients along this direction.

    Returns
    -------
    numpy.array of log-likelihoods for each (dataset, selection coefficient).
    """
    ...
    for selIdx in range(selGrid.shape[0]):
        ...
        # also, no mutation while time elapses for now
        ...
        thisHMM = SelHmm (s1=s1, s2=s2, **kwargs)

        # compute the likelihood for this coefficient
        thisLL[:,selIdx] = thisHMM.computeLogLikelihood (times, samplesSizes, samples)
