...
```shell
$ head examples/ex1_HorseLoci_count.txt
...
$ DiploLocus likelihood --u01 1e-6 --Ne 2500 --gen_time 5 \ 
                ...
                --linear_s1_range="-0.05,0.05,10" \
                --linear_s2_range="-0.05,0.05,10" \
                -o Ex1_MC1R-init_10x10Grid5e-2 
```
The output will be written to `Ex1_MC1R-init_10x10Grid5e-2_LLsurfaces.table`.
...
$ DiploLocus likelihood --u01 1e-6 --Ne 2500 --gen_time 5 \ 
                -i examples/ex1_HorseLoci_count.txt \
                --sample_times_ago 20000,13100,3700,2800,1100,500 \
                --init initFreq --initFreq 2e-4 --t0 7000 --force_t0 \ # Use initial condition of ASIP
                --linear_s1_range="-0.1,0.1,20" \
                --linear_s2_range="-0.1,0.1,20" \
                -o Ex1_ASIP-init_20x20Grid1e-1 \
...
$ DiploLocus likelihood --get_MLR --get_off_grid_max \
            --read_LL_from Ex1_MC1R-init_10x10Grid5e-2_LLsurfaces.table \
            -o Ex1_MC1R-init_10x10Grid5e-2
```
This step will generate output file `Ex1_MC1R-init_10x10Grid5e-2_off-grid_maxLLs.txt`.

Note that when `--get_off_grid_max` is selected, the program will use the Powell algorithm from `scipy.optimize` module starting from the parameters for on-grid max. In the output (with `_off-grid_maxLLs.txt` suffix), the on-grid max will be included as well. If off-grid maximization ran into error, the program would instead report the corresponding value as `np.nan`.
...
```shell
$ ## usage: %prog <input_name> <output_prefix>
$ python examples/plot_contours.py Ex1_MC1R-init_10x10Grid5e-2_LLsurfaces.table Ex1_MC1R-init_10x10Grid5e-2
```
This step will generate png plots named `<output_prefix>_<locusName>_contour.png`. Below is one of them:

[<img src="example_output/Ex1_MC1R-init_10x10Grid5e-2_MC1R_contour.png" width="600"/>](example_output/Ex1_MC1R-init_10x10Grid5e-2_MC1R_contour.png)
...
Data for this example is extracted from [Allen Acient DNA Resources databse v54.1](https://reich.hms.harvard.edu/allen-ancient-dna-resource-aadr-downloadable-genotypes-present-day-and-ancient-dna-data). We identified non-related ancient genomes sampled in UK since 4500 years before present (1950; YBP) and extracted data for 6 SNPs known to be subject to natural selection: rs4988235 (_MCM6_/_LCT_)

To compute log-likelihoods under varying selection strengths for these SNPs, we can run the following command:
```shell
$ DiploLocus likelihood --u01 1.25e-8 --Ne 5000 --gen_time 26.9 \ 
        --vcf examples/ex2_UK_selectLoci_v52_1240K_4500-0YBP_noSG_noRelatives.vcf \
        --info examples/ex2_UK_v52_1240K_4500-0BP_noSG_noRelatives_noContam_minCov0.info \ 
        --init uniform -o Ex2_unifInit_20x20linearGrid5e-1 \
        --linear_s1_range="-0.5,0.5,50" --linear_s2_range="-0.5,0.5,50" \
        --get_off_grid_max --get_MLR -v
```
Here, we assume a per-generation mutation rate of 1.25e-8 per site, and take 5000 as the approximate effective population size. We use a generation time of 26.9 years based on a recent estimate ([Wang et al. 2021](https://www.biorxiv.org/content/10.1101/2021.09.07.459333v1.abstract)), and choose the uniform distribution as a "null" assumption for the initial condition. The resulting likelihood table can be similarly visualized with `examples/plot_contours.py`. For reference, below is the likelihood surface for rs4988235, a SNP regulating the expression of the lactase gene _LCT_: 

[<img src="example_output/Ex2_v54-UK_rs4988235-LCT_unifInit_20x20linearGrid5e-1_contour.png" width="600"/>](example_output/Ex2_v54-UK_rs4988235-LCT_unifInit_20x20linearGrid5e-1_contour.png)
