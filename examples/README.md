...
To download input files to be used in this tutorial, first make sure `diplo-locus` package is installed in your local environment. Then, in the desired working directory, execute

```shell
# only clone the `examples/` folder in the repository
## this command only create the directory for the repository & will not download any file yet
...
From here, all example cases in this tutorial will assume that `example/` is the working directory.
...
The floowing command computes log-likelihoods for both loci on a grid of selection coefficient values using `DiploLocus`:
...
Steinruecken & Song (2014) examined different initial conditions for _MC1R_ and _ASIP_. To obtain log-likelihoods under different initial condition using `Diplolocus`, re-run the program again:
...
This command finished within 7 min on the same machine and will generate plain text files `Ex1_MC1R-init_20x20linGrid1e-1_LLmatrices.table` and `Ex1_MC1R-init_20x20linGrid1e-1_on-grid_maxLLs.txt`. These files are included for reference in `examples/example_output` folder.
...
DiploLocus likelihood --get_MLR --get_on_grid_max \
            ...
            -o Ex1_ASIP-init_50x50linGrid5e-2
```
This step will generate output file `Ex1_ASIP-init_50x50linGrid5e-2_on-grid_maxLLs.txt`. Again, an example files is included in the folder `examples/example_output`.
...
python plot_contours.py Ex1_ASIP-init_50x50linGrid5e-2_LLmatrices.table \
                Ex1_ASIP-init_50x50linGrid5e-2
```
...
Data for this example is extracted from the [Allen Acient DNA Resources databse v54.1](https://reich.hms.harvard.edu/allen-ancient-dna-resource-aadr-downloadable-genotypes-present-day-and-ancient-dna-data). Details on how to download and parse the data can be found [here](https://github.com/steinrue/diplo_locus_manuscript_figs/blob/main/figS13-S14_LCT/README.md). In brief, we identified non-related ancient genomes sampled in UK since 4500 years before present (1950 AC) and genotyped using 1240K enrichment capture. We compiled their SNPs and extracted site within the 2 Mbp genomic region centering on the _MCM6_/_LCT_ locus. Only SNPs with >0.05 pooled minor allele frequency are were maintained. The resulting VCF file `Ex2_UK_v54.1_all_chr2_135e6-137e6.vcf` contains the genotypes of the individuals encoded as pseudo-haploids. The file `Ex2_v54_UK_1240K_noFam_strictPASS_from4500.info` contains the information on time of sampling of the individuals.
...
The user can use the script `plotScores_wGenes.r` to visualize the results. Note that this script reads gene positions from file `hg19_ncbiRefSeq-All_2023May_allFields.txt`. To plot it, one can run:
...
Rscript plotScores_wGenes.r $score_file 2 1363e5 1369e5 pval $pic_name
```
...
We can expand the parameter grid for the specific SNP `rs4988235` with the following command:
...
The resulting likelihood table can be similarly visualized with `plot_contours.py`:
```shell
# script usage: %prog <input_name> <output_prefix>
python plot_contours.py Ex2_v54_UK_rs4988235_unif_50X50linear2e-1_LLmatrices.table.gz \
                Ex2_v54_UK_rs4988235_unif_50X50linear2e-1
```
For reference, below is the likelihood surface for rs4988235, a SNP regulating the expression of the lactase gene _LCT_: 
...
----
<a id="ex3"></a>
### Example 3: Simulating temporal allele count data and plotting the respective trajectories
