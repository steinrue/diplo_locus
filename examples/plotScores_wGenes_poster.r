...
plotManChrs <- function(dt, left, right, statname, colorstat,
  ...
  p <- ggplot(dt)
  chr = unique(dt$Chr)[1]
  p <- p  + geom_point(aes(x=physPos, y=stat, color=colorscale), size=3.5) + xlab(paste('Positions on chromosome', chr))
  ...
  if(mask){
    ...
    p <- p + geom_rect(data=bed, aes(xmin=start, xmax=end, ymin=yMin-width, ymax=yMin, fill=type) )+ 
      ...
      theme(legend.position = 'top', legend.direction = 'horizontal', legend.title=element_text(size=20) )
    }
  }
  ### add colorscale
  ...
  p <- p + scale_y_continuous(statname) + xlim(left, right) + 
...
plotFancyGenes <- function(genes, noRNA=FALSE){
  ...
  p <- p + geom_text_repel(data=Names, aes(y=-10*layer+4, x=pos, label=name), 
                           force=20, size=5, family='Helvetica', fontface='italic',
                           color='black', segment.size=0.3, direction='y', ylim=c(NA, -6) ) 
...
neglogPval_to_LRT <- function(neglogPval){
  pval = 10^(-neglogPval)
  lrt <- qchisq(1-pval, df=1)
  return(lrt)
}
...
plotPeak <- function(ch, inputname, left, right, outputname, statname){
  ...
  if (statname == "MLR"){
    p1 <- plotManChrs(peak[,.(Chr, physPos, stat=MLR, colorscale=s2hat)], 
                      left=left, right=right, statname=TeX(r"($log_{10}LR_{max}$)"),
  ...
  p1 <- p1 + 
    theme(panel.grid.major.x=element_line(linewidth=0.3, color='#cccccc'), 
      ...
      axis.title.y=element_text(size=20, hjust=0.5, vjust=0.5, margin=margin(r=0), family='Arial'), #, angle=0
      axis.text.y=element_text(size=16), axis.text.y.right=element_text(color='gray'), 
      axis.line.y.left=element_line(color='black', linewidth=0.5), #
      axis.line.y.right=element_line(color='lightgray', linewidth=0.5), #
      axis.ticks.y.left=element_line(color='black', linewidth=0.6),
      axis.ticks.y.right=element_line(color='gray', linewidth=0.6),
  ...
  if(nrow(genes)>0){
    ...
    theme(axis.title.y=element_blank(), axis.text.y=element_blank(),#axis.line.x=element_blank(),
      ...
      panel.grid.major.x=element_line(linewidth=0.3, color='#cccccc'),
      panel.grid.minor=element_blank(), panel.grid.major.y=element_blank(),
      axis.line.y.right=element_line(color='gray', linewidth=0.5), #
      axis.line.y.left=element_line(color='black', linewidth=0.5), 
    ...
    pg <- pg  + xlab(paste('Positions on chromosome', ch) ) + theme( axis.title.x=element_text(size=20, hjust=0.5), 
    ...
    ggsave(outputname, plot=pics, width=16, height=7+n*.75, units='in', dpi=500 )
  }else{
    ###No gene in the area, dd x axis back to P1
    p1 <- p1 + theme( axis.title.x=element_text(size=20, hjust=0.5), 
      axis.text.x=element_text(size=16, color='black'),
      axis.line=element_line(color='black', linewidth=0.5), plot.margin=margin(b=5, unit='pt')
      )
    ggsave(outputname, plot=p1, width=16, height=7, units='in', dpi=500) 
  }
